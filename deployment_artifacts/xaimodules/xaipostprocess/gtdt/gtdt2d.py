"""
This module is responsible for linking 2D-ground truth and 2D-prediction in voxel51 dataset.
...
):  # pylint: disable=R0912, R0915, R0914, R0913, W0613
    """Calculate the gtdt field for the given voxel51 2D dataset.
    ...
    # with fo.ProgressBar() as progress_bar:
        for sample in batch_view.iter_samples():  # pylint: disable=R1702
            try:
                ...
                gt_dt = []
                for ground_truth in sample[gt_field].detections:
                    attrs = {}
                    ...
                    # Handle dt attrs and other model-related operations
                    for model in models:
                        ...
                        if ground_truth[model + '_id'] != '':
                            # This is True Positive case
                            if sample[model] is None:
                                continue
                            # if type(sample[model]) != type(None):
                            dts = sample[model].detections
                            for det in dts:
                            ...
                            if update_models is not None and model not in update_models:
                                attrs[model + '_iou'] = \
                                    [
                                        p for p in sample['gt_dt'].detections
                            ...
                            attrs[model + '_bounding_box'] = [0.0, 0.0, 0.0, 0.0]
                ...
                for model in models:
                    ...
                    for det in dts:
                        ...
                        if det[model] == 'fp' or det[model + '_id'] == '':
                            ...
                            attrs[model + '_iou'] = -1.0
                            attrs[model + '_label'] = det['label']
                            attrs[model + '_bounding_box'] = det['bounding_box']
                            attrs[model + '_confidence'] = det['confidence']
                            ...
                            for other_model in models:
                                if other_model != model:
                                    ...
                                    attrs[other_model + '_bounding_box'] = [
                                        0.0,
                                        0.0,
                                        0.0,
                                        0.0,
                ...
                # sample[gt_dt_field] = fo.Detections(detections=gt_dt)
                # gt_dt_dict[sample.id] = fo.Detections(detections=gt_dt)
                # sample_ids.append(sample.id)
                sample_ids_op.append(sample.id)
                sample_gt_dts.append(fo.Detections(detections=gt_dt))
